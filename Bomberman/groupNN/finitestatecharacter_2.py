...
# Second iteration of Main State Machine
class FiniteStateCharacter2(CharacterEntity):
    ...
    def do(self, wrld):
        ...
        #TODO: Find if there is a direct path
        #TODO: Check dangers
            #TODO: Nearby monsters
            #TODO: Nearby Bomb
            #TODO: Nearby explosion
        #TODO: Place Bomb
        ...
        # danger = 0 - No danger
        # danger = 1 - Nearby Monster
        # danger = 2 - Nearby bomb
        # danger = 3 - Nearby explosion
        danger = self.danger_check()

        if len(danger) > 0:

            #TODO - Handle nearby monsters
            #TODO - Handle nearby bomb
            #TODO - Handle nearby explosion

            pass
        else:
            # There is no danger, so attempt to move
            if goTo == None:
                #No Path, so enter placebomb()

                greedyBFS.HeadToExit()
                self.place_bomb()
            else:

                # Move to greedyBFS Coordinate
                self.move(-meX + goTo[0], -meY + goTo[1])

    def place_bomb(self):
        #TODO: Implement placebomb
        pass

    def danger_check(self, wrld, meX, meY):

        danger = 0

        objects = self.checkPerimeter2(wrld, meX, meY)

        if self.isThereMonster(objects):
    ...
    def returnMonsters(self, closeObjects):
        # Return the monsters in close objects

        return [item for item in closeObjects if item[0] == 2]
